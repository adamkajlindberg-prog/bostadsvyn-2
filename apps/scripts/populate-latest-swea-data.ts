...
const upsertData = async (data: T_Api_Response[]) => {
  ...
  for (const item of data) {
    ...
    console.log(
      `Processing latest SWEA with series_id: ${item.seriesId} (${item.shortDescription})`,
    );
    ...
    const response = await fetch(
      `https://api.riksbank.se/swea/v1/Observations/Latest/${item.seriesId}`,
    );
    const observation = (await response.json()) as T_Observation;
