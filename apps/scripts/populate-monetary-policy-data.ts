import { getDbClient, riksbankMonetaryPolicy, riksbankVintages } from "db";
...
const getAllMonetaryPolicy = async () => {
  const response = await fetch(
    "https://api.riksbank.se/monetary_policy_data/v1/forecasts",
  );
...
const upsertData = async (apiData: T_Api_Response) => {
  ...
  for (const item of apiData.data) {
    ...
    console.log(
      `Processing monetary policy with external_id: ${policyData.externalId} (${policyData.description})`,
    );
...
const populateMonetaryPolicyData = async () => {
  ...
  console.log(
    `SUCCESS! Inserted/updated ${monetaryPolicyData.data.length} monetary policy records.`,
  );
};
