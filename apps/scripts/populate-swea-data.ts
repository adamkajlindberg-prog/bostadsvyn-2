import { getDbClient, riksbankSwea, riksbankSweaObservations } from "db";
...
type T_Observation = { date: string; value: number };
...
const upsertData = async (
  ...
  for (const item of data) {
    ...
    console.log(
      `Processing SWEA with series_id: ${item.seriesId} (${item.shortDescription})`,
    );
    ...
    const response = await fetch(
      `https://api.riksbank.se/swea/v1/Observations/${item.seriesId}/${dateFrom}/${dateTo}`,
    );
    const observations = (await response.json()) as T_Observation[];

    // Skip to next series if no observations
    if (!observations || observations.length === 0) {
      console.log(
        "No observations found for this series in the given date range, skipping...",
      );
      continue;
    }
      ...
      await db.transaction(async (tx) => {
        for (const observation of batch) {
          await tx
            ...
            .onConflictDoUpdate({
              target: [
                riksbankSweaObservations.sweaId,
                riksbankSweaObservations.date,
              ],
              set: {
                value: observation.value,
                updatedAt: new Date(),
