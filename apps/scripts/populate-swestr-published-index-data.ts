...
const populateSwestrPublishedIndexData = async (
  dateFrom: string,
  dateTo: string,
) => {
  if (!dateFrom || !dateTo) {
  ...
  console.log(
    "Retrieving data from Riksbank API (SWESTR - Published Index)...",
  );
  console.log(`Using date range: ${dateFrom} to ${dateTo}`);

  const response = await fetch(
    `https://api.riksbank.se/swestr/v1/index/SWESTRINDEX?fromDate=${dateFrom}&toDate=${dateTo}`,
  );
  const data = (await response.json()) as T_Published_Index[];
  ...
  console.log(
    "SUCCESS! All SWESTR published index data populated successfully.",
  );
};
