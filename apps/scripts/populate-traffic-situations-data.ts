import { getDbClient, trafficSituations } from "db";
...
const getExistingData = async () => {
  const data = await db
    .select({ trafficId: trafficSituations.trafficId })
...
const insertBatch = async (batch: T_Traffic_Situation[]) => {
  await db.transaction(async (tx) => {
    for (const item of batch) {
      ...
      const dataToEmbed = [item.locationDescriptor, item.roadName]
        .filter(Boolean)
        .join(", ");
...
const populateTrafficSituationsData = async () => {
  try {
    ...
    const freshSituations = allSituations.filter(
      (situation: T_Traffic_Situation) =>
        !existingData.includes(situation.trafficId),
    );

    const batches: T_Traffic_Situation[][] = chunkArray(
      freshSituations,
      BATCH_SIZE,
    );
    ...
    for (const batchEvents of batches) {
      ...
      console.log(
        `Inserting traffic situations batch ${batchNumber + 1}/${totalBatches} (${batchEvents.length} situations)...`,
      );

      await insertBatch(batchEvents);
      console.log(
        `Inserted traffic situations batch ${batchNumber + 1}/${totalBatches}.`,
      );
