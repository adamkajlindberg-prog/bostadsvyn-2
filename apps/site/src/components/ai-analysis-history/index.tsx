...
export function AIAnalysisHistory() {
  ...
  const loadHistory = async () => {
    try {
      ...
      const [valuationResponse, analysisResponse] = await Promise.all([
        fetch("/api/ai/valuation-history"),
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/ai/${itemToDelete.type === "valuation" ? "valuation" : "analysis"}-history/${itemToDelete.id}`,
        {
          method: "DELETE",
        },
      );
      ...
      if (!response.ok) {
        throw new Error("Kunde inte radera historikpost");
      }
