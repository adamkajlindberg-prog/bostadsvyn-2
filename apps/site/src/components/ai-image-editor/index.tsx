...
const ImageEditor = () => {
  ...
  const { selectedImage, setSelectedImage } = useSelectImageStore(
    (state) => state,
  );
  ...
  const handleSubmit = () => {
    ...
    if (
      ...
      image.src.src.trim().length > 0
    ) {
      parts.push({
        type: "file" as const,
        mediaType: "image/png",
        url: `${env.NEXT_PUBLIC_WEB_URL}${image.src.src}`,
      });
      lastSentImageIndex.current = selectedImage;
    }
  ...
  return (
    <>
              ...
              <CardContent className="px-0 h-full flex flex-col">
                <div className="p-6">
                  <div className="flex flex-wrap items-center gap-2 pb-1.5">
                    ...
                    <h3 className="text-2xl font-semibold tracking-tight leading">
                      Fastighetsbilder
                    </h3>
                  </div>
                ...
                <div className="space-y-4 p-6 border-t overflow-y-auto">
                  {propertyImages.map((image) => (
                    ...
                    >
                      <Image
                        src={image.src}
                        alt={`Fastighetsbild ${image.index}`}
                        fill
                        sizes="100%"
                        className="object-cover"
                      />
                      <div className="absolute top-3 left-3 text-xs font-medium bg-nordic-snow text-primary px-3 py-1 rounded-full">
                        Bild {image.index}
                      </div>
                  ...
                  <div className="flex flex-col @xl:flex-row @xl:justify-between items-start">
                    <div>
                      <div className="flex flex-wrap items-center gap-2 mb-1.5">
                        ...
                        <h3 className="text-xl @lg:text-2xl font-semibold tracking-tight leading">
                          AI Renoveringsassistent
                        </h3>
                      </div>

                      <p className="text-sm text-muted-foreground">
                        Beskriv vad du vill ändra på den valda bilden
                      </p>
                    </div>
                    ...
                    <ConversationContent className="py-2 pl-0.5 pr-0">
                      <Message from="assistant" className="items-start">
                        <MessageContent className="group-[.is-assistant]:bg-muted">
                          Välkommen till AI-bildredigeraren för "Exklusiv villa
                          med havsutsikt"! 🏠✨ <br />
                          <br />
                          Jag kan hjälpa dig visualisera hur fastigheten skulle
                          kunna se ut med olika förändringar: <br />
                          <br />
                          <span>
                            🎨 <strong>Måla om väggar</strong> - "Måla
                            vardagsrummet i ljusblått"
                          </span>
                          <span>
                            🪑 <strong>Möblera om</strong> - "Lägg till en stor
                            soffa och ett soffbord"
                          </span>
                          <span>
                            💡 <strong>Ändra belysning</strong> - "Lägg till
                            varmare belysning"
                          </span>
                          <span>
                            🔧 <strong>Renovera</strong> - "Gör ett öppet kök
                            mot vardagsrummet"
                          </span>
                          <span>
                            🌿 <strong>Dekorera</strong> - "Lägg till växter och
                            tavlor"
                          </span>{" "}
                          <br />
                          Den första bilden är vald som standard, men du kan
                          välja vilken annan bild du föredrar. <br />
                          <br />
                          När du har valt en bild, låt oss veta vilka ändringar
                          du vill göra! <br />
                          <br />
                          <span>
                            💡 <strong>Tips</strong>: Logga in för att spara och
                            dela dina AI-redigeringar automatiskt!
                          </span>
                        </MessageContent>
                          ...
                          <MessageContent className="group-[.is-assistant]:bg-muted">
                            {message.parts.map((part, i) => {
                              switch (part.type) {
                                ...
                                case "file":
                                  if (
                                    part.mediaType.startsWith("image/png") &&
                                    message.role === "assistant"
                                  ) {
                                    return (
                                      <div
                                        key={`image-${message.id}-${i}`}
                                        className="aspect-video h-32 @lg:min-h-64 relative group"
                                      >
                                        <Image
                                          src={part.url}
                                          alt="AI-bild"
      ...
      <ImageFullModal
        beforeImage={imagePreview?.before ?? ""}
        afterImage={imagePreview?.after ?? ""}
      />
    </>
  );
};
