...
import { BuildingIcon, FactoryIcon, StoreIcon, TrendingUpIcon } from "lucide-react"
    ...
    return (
        <div className="grid grid-cols-1 @2xl:grid-cols-2 @5xl:grid-cols-4 gap-6 mb-14">
            {highlights.map((highlight) => (
                <Card key={highlight.title} className="py-6 shadow-xs border-primary/40">
                    <CardContent className="px-6">
                        ...
                        <h5 className="text-lg text-center font-semibold mb-2">{highlight.title}</h5>
                        <p className="text-sm text-center text-muted-foreground mb-4">{highlight.description}</p>
                        
                        <div className="flex justify-center">
                            <div className="bg-primary/10 text-xs text-center text-primary font-medium rounded-full px-3 py-1">{highlight.badgeText}</div>
                        </div>
                    </CardContent>
                </Card>
            ))}
