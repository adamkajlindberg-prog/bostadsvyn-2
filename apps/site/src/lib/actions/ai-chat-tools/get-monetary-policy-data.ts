...
const getMonetaryPolicyData = async (question: string) => {
  try {
    ...
    const policyRounds = await fetch(
      `https://api.riksbank.se/monetary_policy_data/v1/forecasts/policy_rounds`,
      {
        method: "GET",
        headers: {
    ...
    const response = await fetch(
      `https://api.riksbank.se/monetary_policy_data/v1/forecasts?policy_round_name=${policyRoundName}&series=${seriesId}`,
      {
        method: "GET",
        headers: {
          Accept: "application/json",
