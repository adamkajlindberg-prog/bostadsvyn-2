...
const getPublishedIndex = async (question: string ) => {
    ...
    // Fetch published index data
    try {
        const response = await fetch(`https://api.riksbank.se/swestr/v1/index/SWESTRINDEX?fromDate=${date}`, {
            method: 'GET',
            headers: {
...
export default getPublishedIndex