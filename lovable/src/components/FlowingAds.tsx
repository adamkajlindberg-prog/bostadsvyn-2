...
import { Wand2, Loader2, Heart, Share2, ExternalLink, Crown, Star, TrendingUp, Sparkles } from 'lucide-react';
...
const FlowingAdsGrid = () => {
  ...
  const ads: AdData[] = [{
    ...
    type: "Lägenhet",
    image: premiumRoom1,
    images: [premiumRoom1, premiumRoom2, premiumRoom3, premiumRoom4, premiumRoom5, premiumRoom6, premiumRoom7, premiumRoom8, premiumRoom9, premiumRoom10],
    features: ["Takterrass", "Hiss", "Panoramautsikt", "Gym", "Concierge"],
    size: "180 m²",
    description: "Lyxig penthouse med spektakulär takterrass och panoramautsikt över Stockholm. Fullständigt renoverad med högsta standard och exklusiva materialval.",
    link: "/property/1",
    adTier: 'premium',
    priorityScore: 100,
    isFeatured: true
    ...
    size: "350 m²",
    description: "En magnifik villa med privat pool och fantastisk sjöutsikt. Perfekt för familjer som värdesätter lyx och komfort.",
    link: "/property/2",
    adTier: 'plus',
    priorityScore: 80
  }, {
    ...
    size: "140 m²",
    description: "Perfekt för familjen med egen trädgård och moderna lösningar. Nyligen renoverat med smakfulla detaljer.",
    link: "/property/4",
    adTier: 'plus',
    priorityScore: 70
  }, {
  ...
  const generateAIImage = async (adId: number) => {
    ...
    try {
      const {
        data,
        error
      } = await supabase.functions.invoke('ai-image-generator', {
        body: {
          prompt: prompt,
  ...
  const getAdSizeClass = (adTier: AdTier, index: number) => {
    switch (adTier) {
      case 'premium':
  ...
  const getTierBadge = (adTier: AdTier) => {
    switch (adTier) {
      case 'premium':
        return <Badge className="bg-gradient-to-r from-amber-500 to-orange-500 text-white border-0">
  ...
  const canUseAIFeatures = (adTier: AdTier) => adTier === 'premium';
  return null;
};
