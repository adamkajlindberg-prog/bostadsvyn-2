...
export const LocationAutocomplete: React.FC<LocationAutocompleteProps> = ({
  ...
  const fetchSuggestions = async (queryRaw: string | null | undefined) => {
    ...
    try {
      ...
      const {
        data: areas
      } = await supabase.from('geographic_areas').select('id, name, area_type, center_lat, center_lng').ilike('name', `%${query}%`).in('area_type', ['municipality', 'district']).limit(8);
      ...
      if (areas && areas.length > 0) {
        areas.forEach(area => {
          ...
          const muniName = isMunicipality ? `${baseName.replace(/\s*kommun\s*$/i, '').trim()} kommun` : baseName;
      ...
      try {
        const {
          data: properties
        } = await supabase.from('properties').select('address_street, address_city, latitude, longitude').or(`address_street.ilike.%${query}%, address_city.ilike.%${query}%`).limit(6);

        // Search property sales history for additional addresses
        const {
          data: salesHistory
        } = await supabase.from('property_sales_history').select('address_street, address_city, latitude, longitude').or(`address_street.ilike.%${query}%, address_city.ilike.%${query}%`).limit(6);
        ...
        if (properties) {
          properties.forEach(prop => {
            ...
            if (!addressSet.has(addressKey) && prop.address_street.toLowerCase().includes(query.toLowerCase())) {
        ...
        if (salesHistory) {
          salesHistory.forEach(sale => {
            ...
            if (!addressSet.has(addressKey) && sale.address_street.toLowerCase().includes(query.toLowerCase())) {
              addressSet.add(addressKey);
              allSuggestions.push({
        ...
        console.log('No properties found, continuing with geographic areas only');
      }

      // Augment with OSM suggestions via edge function for broader coverage (Täby, Vasastan, etc.)
      try {
        const lower = query.toLowerCase();
        const typeHint = lower.includes('kommun') ? 'municipality' : lower.includes('stadsdel') || lower.includes('område') ? 'district' : undefined;
        const {
          data,
          error
        } = await supabase.functions.invoke('osm-boundary', {
          body: {
            q: query,
            list: true,
        ...
        if (error) {
          console.warn('OSM suggestions error:', (error as any).message || error);
        } else if (data && Array.isArray((data as any).suggestions)) {
          const suggestionsFromOsm = (data as any).suggestions as LocationSuggestion[];
      ...
      const toKey = (s: LocationSuggestion) => {
        if (s.type === 'municipality') {
          const base = s.name.replace(/\s*kommun\s*$/i, '').trim().toLowerCase();
          return `muni|${base}`;
        }
        if (s.type === 'district') {
      ...
      const municipalities = normalized.filter(s => s.type === 'municipality');
  ...
  return <div className="relative">
      ...
      {open && (suggestions.length > 0 || loading) && <div className="absolute top-full left-0 right-0 z-50 mt-1 bg-popover border border-border rounded-md shadow-lg max-h-[340px] overflow-y-auto">
          {loading && <div className="p-3 text-center text-sm text-muted-foreground">
              Söker...
            </div>}
          {!loading && suggestions.length > 0 && <div>
              {(() => {
          const municipalities = suggestions.filter(s => s.type === 'municipality');
          const districts = suggestions.filter(s => s.type === 'district');
          const addresses = suggestions.filter(s => s.type === 'address');
          const renderGroup = (label: string, items: LocationSuggestion[]) => items.length > 0 && <div>
                      <div className="px-3 pt-2 pb-1 text-[10px] font-medium text-muted-foreground uppercase tracking-wide">{label}</div>
                      {items.map(location => {
              const Icon = getLocationIcon(location.type);
              return <div key={location.id} onClick={() => handleSelect(location)} className="flex items-center gap-3 p-3 cursor-pointer hover:bg-accent hover:text-accent-foreground border-b border-border last:border-b-0">
                            <Icon className="h-4 w-4 text-muted-foreground flex-shrink-0" />
                            <div className="flex-1 min-w-0">
                              <div className="font-medium text-sm">{location.name}</div>
                              <div className="text-xs text-muted-foreground">
                                {location.municipality && location.municipality !== location.name && <span>{location.municipality} • </span>}
      ...
      {/* No results message */}
      {open && !loading && suggestions.length === 0 && inputValue.length >= 1 && <div className="absolute top-full left-0 right-0 z-50 mt-1 bg-popover border border-border rounded-md shadow-lg p-3">
          <div className="text-sm text-muted-foreground text-center">Inga platser hittades.</div>
        </div>}
    </div>;
};