...
const comparisonFields = [
  { key: 'price', label: 'Pris (SEK)', icon: DollarSign, format: (value: number) => value.toLocaleString('sv-SE') + ' kr' },
  { key: 'property_type', label: 'Typ', icon: Home },
  { key: 'living_area', label: 'Boarea (m²)', icon: Ruler, format: (value: number) => value + ' m²' },
  { key: 'plot_area', label: 'Tomtarea (m²)', icon: Ruler, format: (value: number) => value + ' m²' },
  ...
  { key: 'monthly_fee', label: 'Månadsavgift', icon: DollarSign, format: (value: number) => value.toLocaleString('sv-SE') + ' kr/mån' },
      ...
      <Card>
        ...
        <CardContent className="text-center py-12">
          ...
          <p className="text-muted-foreground mb-4">
            Lägg till fastigheter för att jämföra deras egenskaper sida vid sida.
          </p>
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">
            Jämför {properties.length} fastighet{properties.length !== 1 ? 'er' : ''}
          </p>
        </div>
      ...
      <div className="overflow-x-auto">
        <div className="min-w-max">
          {/* Property Headers */}
          <div className="grid gap-4 mb-6" style={{ gridTemplateColumns: `300px repeat(${properties.length}, 250px)` }}>
            <div></div>
                ...
                {comparisonFields.map((field, index) => {
                  ...
                  return (
                    <div
                      ...
                      style={{ gridTemplateColumns: `300px repeat(${properties.length}, 250px)` }}
                    >
                        ...
                        return (
                          <div
                            ...
                            className={`flex items-center justify-center p-2 rounded ${
                              highlighted ? 'bg-success/10 text-success font-semibold' : ''
                            }`}
                          >
                  ...
                  {properties.map((property) => (
                    <div key={property.id} className="space-y-1">
                      {property.features && property.features.length > 0 ? (
                        ...
                        <span className="text-muted-foreground text-sm">Inga angivna</span>
                      )}
                    </div>
                  ))}
                </div>
                  ...
                  return (
                    <div key={property.id} className="text-center space-y-2">
                      ...
                      <div className="space-y-1 text-xs text-muted-foreground">
                        {pricePerSqm && (
                          <div>Pris per m²: {pricePerSqm.toLocaleString('sv-SE')} kr</div>
                        )}
