...
import { Heart, Share2, MapPin, Calendar, Ruler, BedDouble, Bath, Home, ArrowLeft, Phone, Mail, Eye, ChevronLeft, ChevronRight, Maximize, X, MessageCircle, DollarSign, FileText, Check, Zap, Car, Wifi, ShoppingBag, Train, School, Dumbbell } from 'lucide-react';
...
export const RentalDetails: React.FC = () => {
  ...
  const loadProperty = async () => {
    try {
      ...
      const {
        data: propertyData,
        error: propertyError
      } = await supabase.from('properties').select('*').eq('id', id).eq('status', 'FOR_RENT').maybeSingle();
      if (!propertyData) {
  ...
  const getTestRentalProperty = (propertyId: string): RentalProperty | null => {
    const testProperties: RentalProperty[] = [{
      ...
      bedrooms: 2,
      bathrooms: 1,
      images: ['https://images.unsplash.com/photo-1560448204-e02f11c3d0e2?w=1200&h=675&fit=crop'],
      ...
      rental_info: {
        ...
        neighborhood_description: 'Centralt läge med närhet till shopping, restauranger och kollektivtrafik.',
        nearest_metro: 'T-Centralen (3 min promenad)',
        transport_description: 'Perfekt kommunikation med alla tunnelbanelinjer och bussar.',
      ...
      bedrooms: 1,
      bathrooms: 1,
      images: ['https://images.unsplash.com/photo-1522708323590-d24dbb6b0267?w=1200&h=675&fit=crop'],
      created_at: new Date().toISOString(),
      updated_at: new Date().toISOString(),
      user_id: 'test',
      ...
      rental_info: {
        ...
        max_occupants: 2,
        references_required: true,
        neighborhood_description: 'Modernt område vid vattnet med fin utsikt och bra grönområden.',
  ...
  const checkFavoriteStatus = async () => {
    ...
    try {
      const {
        data
      } = await supabase.from('property_favorites').select('id').eq('user_id', user.id).eq('property_id', id).single();
      setIsFavorite(!!data);
    } catch (error) {
      setIsFavorite(false);
    }
    ...
    return <div className="container mx-auto px-4 py-8">
        <Card>
          <CardContent className="text-center py-12">
            ...
            <h3 className="text-lg font-semibold mb-2">Hyresbostad hittades inte</h3>
  ...
  return <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-6">
              ...
              {property.rental_info && <>
                  <div>
                    <h3 className="font-semibold mb-4 text-lg">Hyresinformation</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      {property.rental_info.lease_duration && <div className="flex items-start gap-3 p-3 rounded-lg bg-muted/50">
                      ...
                      <div className="flex items-start gap-3 p-3 rounded-lg bg-muted/50">
                        ...
                        <div>
                          ...
                          <div className="text-sm text-muted-foreground">
                            {property.rental_info.pets_allowed ? 'Tillåtna' : 'Ej tillåtna'}
                          </div>
                        </div>
                      </div>
                      <div className="flex items-start gap-3 p-3 rounded-lg bg-muted/50">
                        ...
                        <div>
                          ...
                          <div className="text-sm text-muted-foreground">
                            {property.rental_info.furnished ? 'Möblerad' : 'Omöblerad'}
                          </div>
                        </div>
                      </div>
                      {property.rental_info.smoking_allowed !== undefined && <div className="flex items-start gap-3 p-3 rounded-lg bg-muted/50">
                          <Check className="h-5 w-5 text-primary mt-0.5" />
                          <div>
                            <div className="font-medium">Rökning</div>
                            <div className="text-sm text-muted-foreground">
                              {property.rental_info.smoking_allowed ? 'Tillåten' : 'Ej tillåten'}
                            </div>
                          </div>
                        </div>}
                      {property.rental_info.is_shared !== undefined && <div className="flex items-start gap-3 p-3 rounded-lg bg-muted/50">
                          <Home className="h-5 w-5 text-primary mt-0.5" />
                          <div>
                            <div className="font-medium">Delad bostad</div>
                            <div className="text-sm text-muted-foreground">
                              {property.rental_info.is_shared ? 'Ja (inneboende)' : 'Nej'}
                            </div>
                          </div>
                        </div>}
                    </div>
              ...
              {property.rental_info && (property.rental_info.floor_level || property.rental_info.has_elevator !== undefined || property.year_built || property.rental_info.energy_rating) && <>
                  <div>
                    <h3 className="font-semibold mb-4 text-lg">Byggnadsinformation</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {property.rental_info.floor_level && <div className="flex items-start gap-3 p-3 rounded-lg bg-muted/50">
                          <Home className="h-5 w-5 text-primary mt-0.5" />
                          <div>
                      ...
                      {property.rental_info.has_elevator !== undefined && <div className="flex items-start gap-3 p-3 rounded-lg bg-muted/50">
                          <Check className="h-5 w-5 text-primary mt-0.5" />
                          <div>
                            <div className="font-medium">Hiss</div>
                            <div className="text-sm text-muted-foreground">
                              {property.rental_info.has_elevator ? 'Ja' : 'Nej'}
                            </div>
                          </div>
                        </div>}
                      {property.year_built && <div className="flex items-start gap-3 p-3 rounded-lg bg-muted/50">
                          <Calendar className="h-5 w-5 text-primary mt-0.5" />
                          <div>
                      ...
                      {property.rental_info.energy_rating && <div className="flex items-start gap-3 p-3 rounded-lg bg-muted/50">
          ...
          <Card>
            <CardContent className="space-y-6 pt-6">
              ...
              {property.rental_info && <div>
                  <h3 className="font-semibold mb-4 text-lg">Vad ingår i hyran?</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {property.rental_info.utilities_included && <div className="flex items-center gap-3 p-3 rounded-lg bg-muted/30">
                        <Zap className="h-5 w-5 text-primary" />
                        <span className="text-sm">El och värme ingår</span>
                      </div>}
                    {property.rental_info.internet_included && <div className="flex items-center gap-3 p-3 rounded-lg bg-muted/30">
                        <Wifi className="h-5 w-5 text-primary" />
                        <span className="text-sm">Bredband ingår</span>
                      </div>}
                    {property.rental_info.has_balcony && <div className="flex items-center gap-3 p-3 rounded-lg bg-muted/30">
                        <Home className="h-5 w-5 text-primary" />
                        <span className="text-sm">Balkong</span>
                      </div>}
                    {property.rental_info.has_garden && <div className="flex items-center gap-3 p-3 rounded-lg bg-muted/30">
                        <Home className="h-5 w-5 text-primary" />
                        <span className="text-sm">Trädgård</span>
                      </div>}
                    {property.rental_info.parking_available && <div className="flex items-center gap-3 p-3 rounded-lg bg-muted/30">
                        <Car className="h-5 w-5 text-primary" />
                        <span className="text-sm">
                          {property.rental_info.parking_type || 'Parkering tillgänglig'}
                        </span>
                      </div>}
                  </div>
                </div>}
              ...
              {/* Nearby Services */}
              {property.rental_info && (property.rental_info.neighborhood_description || property.rental_info.nearest_metro || property.rental_info.transport_description) && <>
                  <div>
                    <h3 className="font-semibold mb-4 text-lg">Område och transport</h3>
                    <div className="space-y-3">
                      {property.rental_info.neighborhood_description && <div>
                          <p className="text-sm text-muted-foreground">
                            {property.rental_info.neighborhood_description}
                          </p>
                        </div>}
                      {property.rental_info.nearest_metro && <div className="flex items-start gap-3">
                          <Train className="h-5 w-5 text-primary mt-0.5" />
                          <div>
                            <div className="font-medium text-sm">Närmaste kollektivtrafik</div>
                            <div className="text-sm text-muted-foreground">
                              {property.rental_info.nearest_metro}
                            </div>
                          </div>
                        </div>}
                      {property.rental_info.transport_description && <div className="flex items-start gap-3">
                          <Train className="h-5 w-5 text-primary mt-0.5" />
                          <div>
                            <div className="font-medium text-sm">Kommunikationer</div>
                            <div className="text-sm text-muted-foreground">
                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          <div className="font-medium text-sm">Träning & Fritid</div>
                          <div className="text-sm text-muted-foreground">
        ...
        <div className="space-y-6">
          <RentalChat propertyId={property.id} propertyOwnerId={property.user_id} propertyOwner={propertyOwner || undefined} />
