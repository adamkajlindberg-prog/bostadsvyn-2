import { Calendar, Home, MapPin, TrendingUp } from "lucide-react";
...
export const SearchResults: React.FC<SearchResultsProps> = ({
  ...
  const [_columnsPerRow, _setColumnsPerRow] = useState<string>("2");
  ...
  const _searchCity = getCityFromLocation(searchLocation);
  ...
  const fetchSearchResults = async () => {
    ...
    try {
      ...
      const { data: exactMatches, error: exactError } = await supabase
        ...
        .select("*")
        .or(
          `address_city.ilike.%${searchLocation}%,address_street.ilike.%${searchLocation}%`,
        )
      ...
      // Separate by status
      const forSale = (exactMatches || []).filter(
        (p) => p.status === "FOR_SALE",
      ) as Property[];
      const comingSoon = (exactMatches || []).filter(
      ...
      if (forSale.length === 0 && comingSoon.length === 0) {
        ...
        const altForSale = (alternatives || []).filter(
          (p) => p.status === "FOR_SALE",
        ) as Property[];
        const altComingSoon = (alternatives || []).filter(
          (p) => p.status === "COMING_SOON",
        ) as Property[];
      ...
      <div className="mb-8">
        <div className="flex items-center gap-2 mb-2">
          ...
          <h1 className="text-3xl font-bold">
            {showingAlternatives
              ? `Liknande bostäder nära ${searchLocation}`
              : `Sökresultat för ${searchLocation}`}
          </h1>
        </div>
        {showingAlternatives && (
          <div className="bg-muted/50 border border-border rounded-lg p-4 mb-4">
            <p className="text-sm">
              <strong>Din specifika sökning gav inga resultat.</strong> Vi visar
              istället liknande bostäder i närområdet som kan passa dig.
            </p>
          </div>
        )}
        <p className="text-muted-foreground">
          {forSaleProperties.length + comingSoonProperties.length} aktiva
          annonser • {salesHistory.length} slutpriser
        </p>
      </div>
            ...
            <Card>
              <CardContent className="text-center py-8">
                ...
                <h3 className="text-lg font-semibold mb-2">
                  Inga fastigheter till salu
                </h3>
                <p className="text-muted-foreground">
                  Det finns för närvarande inga fastigheter till salu i{" "}
                  {searchLocation}.
                </p>
              </CardContent>
            </Card>
          ...
          {comingSoonProperties.length > 0 ? (
            ...
            <Card>
              <CardContent className="text-center py-8">
                ...
                <h3 className="text-lg font-semibold mb-2">
                  Inga kommande fastigheter
                </h3>
                <p className="text-muted-foreground">
                  Det finns för närvarande inga kommande fastigheter i{" "}
                  {searchLocation}.
                </p>
              </CardContent>
            </Card>
          ...
          {salesHistory.length > 0 ? (
                    ...
                    <div className="flex items-start justify-between">
                      ...
                      <Badge variant="outline" className="text-xs">
                        {sale.property_type === "apartment"
                          ? "Lägenhet"
                          : sale.property_type === "house"
                            ? "Villa"
                            : sale.property_type === "townhouse"
                              ? "Radhus"
                              : sale.property_type}
                      </Badge>
                    </div>
            ...
            <Card>
              <CardContent className="text-center py-8">
                ...
                <p className="text-muted-foreground">
                  Det finns inga registrerade slutpriser för {searchLocation}.
                </p>
              </CardContent>
            </Card>
          )}
