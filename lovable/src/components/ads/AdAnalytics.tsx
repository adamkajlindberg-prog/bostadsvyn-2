...
const AdAnalytics = () => {
  ...
  const fetchAnalytics = async () => {
    try {
      ...
      const filteredAnalytics = tierFilter === 'all' 
        ? mockAnalytics 
  ...
  const exportData = () => {
    const csvContent = [
      ['Ad Title', 'Tier', 'Views', 'Clicks', 'CTR %', 'Conversion Rate %', 'Avg. Time on Page (s)'],
      ...analytics.map(ad => [
        ad.ad_title,
        ad.ad_tier,
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Track performance and optimize your advertising strategy</p>
        </div>
              ...
              <div key={ad.ad_id} className="border rounded-lg p-4 space-y-3">
                ...
                <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                  <div className="text-center p-3 bg-muted rounded-lg">
                    <p className="text-2xl font-bold text-primary">{ad.views_count}</p>
                  ...
                  <div className="text-center p-3 bg-muted rounded-lg">
                    <p className="text-2xl font-bold text-orange-600">{ad.ctr.toFixed(1)}%</p>
                    <p className="text-sm text-muted-foreground">CTR</p>
                  </div>
                  
                  <div className="text-center p-3 bg-muted rounded-lg">
                    <p className="text-2xl font-bold text-purple-600">{ad.conversion_rate.toFixed(1)}%</p>
                    <p className="text-sm text-muted-foreground">Conversion</p>
                  </div>
                  
                  <div className="text-center p-3 bg-muted rounded-lg">
                    <p className="text-2xl font-bold text-red-600">{ad.avg_time_on_page}s</p>
                ...
                <div className="flex items-center justify-between pt-2">
                  <div className="flex items-center gap-2">
                    <div className={`w-3 h-3 rounded-full ${
                      ad.ctr > 5 ? 'bg-green-500' : ad.ctr > 2 ? 'bg-yellow-500' : 'bg-red-500'
                    }`} />
                    <span className="text-sm">
                      {ad.ctr > 5 ? 'High Performance' : ad.ctr > 2 ? 'Average Performance' : 'Needs Improvement'}
                    </span>
                  </div>
        ...
        <CardContent className="space-y-3">
          <div className="flex items-start gap-3 p-3 bg-nordic-ice rounded-lg">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">Use compelling headlines and high-quality images to increase click-through rates</p>
            </div>
          </div>
          ...
          <div className="flex items-start gap-3 p-3 bg-green-50 rounded-lg">
            ...
            <div>
              ...
              <p className="text-sm text-green-700">Consider upgrading to premium for featured placement and increased visibility</p>
            </div>
          </div>
          ...
          <div className="flex items-start gap-3 p-3 bg-orange-50 rounded-lg">
            ...
            <div>
              ...
              <p className="text-sm text-orange-700">Add more detailed descriptions and virtual tours to increase engagement time</p>
            </div>
          </div>
        </CardContent>
      </Card>
