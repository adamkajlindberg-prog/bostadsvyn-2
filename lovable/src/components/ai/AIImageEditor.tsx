...
const editPresets = [
  { id: 'wall-color', label: 'Ändra väggfärg', icon: Palette, prompt: 'Change the wall color to', editType: 'generation' as const },
  { id: 'furniture', label: 'Möblera om', icon: Sofa, prompt: 'Rearrange the furniture in this room to', editType: 'generation' as const },
  { id: 'lighting', label: 'Ändra belysning', icon: Sun, prompt: 'Change the lighting in this room to', editType: 'generation' as const },
  { id: 'flooring', label: 'Golv & mattor', icon: Home, prompt: 'Change the flooring to', editType: 'generation' as const },
  { id: 'garden', label: 'Trädgård & utemiljö', icon: TreePine, prompt: 'Add to the outdoor space', editType: 'generation' as const },
  { id: 'garage', label: 'Garage & carport', icon: Car, prompt: 'Add or modify garage/parking to include', editType: 'generation' as const },
  { id: 'remove-bg', label: 'Ta bort bakgrund', icon: Scissors, prompt: 'Remove background', editType: 'object_removal' as const },
  { id: 'inpaint', label: 'Precisionsredigering', icon: Target, prompt: 'Precisely edit selected area:', editType: 'inpainting' as const },
  { id: 'style-transfer', label: 'Stilöverföring', icon: Layers, prompt: 'Apply style', editType: 'style_transfer' as const },
  { id: 'smart-mask', label: 'AI-maskering', icon: Brush, prompt: 'AI-assisted precise editing', editType: 'inpainting' as const },
  { id: 'generate-video', label: '🎬 Videotour', icon: Video, prompt: 'Create virtual tour video', editType: 'video_generation' as const },
  { id: 'generate-3d', label: '🏠 3D-vy', icon: Box, prompt: 'Generate 3D room visualization', editType: '3d_generation' as const },
];
...
const qualityLevels = [
  { id: 'standard', label: 'Standard', description: 'Snabb, grundläggande kvalitet' },
  { id: 'high', label: 'Hög', description: 'Balans mellan kvalitet och hastighet' },
  { id: 'ultra', label: 'Ultra', description: 'Högsta kvalitet, tar längre tid' }
];

export default function AIImageEditor({ propertyImages = [], onImagesUpdated }: AIImageEditorProps) {
  const { user } = useAuth();
  const [selectedImage, setSelectedImage] = useState<string>('');
  const [editPrompt, setEditPrompt] = useState('');
  const [customPrompt, setCustomPrompt] = useState('');
  const [negativePrompt, setNegativePrompt] = useState('');
  const [editType, setEditType] = useState<'preset' | 'custom' | 'room'>('preset');
  ...
  const fetchSavedProperties = async () => {
    ...
    try {
      ...
      const propertiesWithImages = favorites
        ?.filter(fav => fav.properties && fav.properties.images && fav.properties.images.length > 0)
        .map(fav => ({
  ...
  const handleBackgroundRemoval = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: error.message || "Kunde inte ta bort bakgrund. Försök med AI-redigering istället.",
        variant: "destructive",
      });
    } finally {
  ...
  const removeBackgroundInBrowser = async (imageFile: File): Promise<string> => {
    try {
  ...
  const generateEditedImage = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: error.message || "Kunde inte redigera bilden. Försök igen.",
        variant: "destructive",
      });
    } finally {
      setIsGenerating(false);
      ...
      try {
        ...
        const { data } = await supabase.functions.invoke('ai-image-editor', {
          body: {
            ...
            prompt: customPrompt || 'Professional real estate photography enhancement',
            editType: 'generation',
            stylePreset: selectedStyle,
            qualityLevel: selectedQuality
          }
  ...
  const handleBulkExport = async (format: string, quality?: number) => {
    try {
      toast({
      ...
      if (format === 'zip') {
        ...
        for (let i = 0; i < generatedImages.length; i++) {
          await handleDownload(generatedImages[i], `ai-edited-${i + 1}.${format}`);
        }
      }
  ...
  const handleQuickAction = async (action: string) => {
    ...
    const quickPrompts = {
      'enhance': 'Professional real estate photography enhancement, perfect lighting, sharp details, vibrant colors',
      'remove-bg': 'Remove background completely, transparent background',
      'lighting': 'Perfect natural lighting, bright and welcoming atmosphere, professional real estate lighting',
      'staging': 'Professional home staging, modern furniture arrangement, clean and inviting interior design'
    };
    ...
    try {
      ...
      if (data.success) {
        ...
        toast({
          ...
          description: `${action === 'enhance' ? 'Förbättring' : 
                        action === 'remove-bg' ? 'Bakgrundsborttagning' :
                        action === 'lighting' ? 'Ljusförbättring' : 'Staging'} slutförd.`,
        });
      } else {
        throw new Error(data.error);
      }
  ...
  const handleDownload = async (imageUrl: string, filename: string, format?: string, quality?: number) => {
    try {
      ...
      if (format && format !== 'png' && !imageUrl.startsWith('data:')) {
        ...
        finalBlob = await new Promise<Blob>((resolve) => {
          canvas.toBlob(
            (blob) => resolve(blob!),
            `image/${format}`,
            format === 'jpg' || format === 'jpeg' ? (quality || 90) / 100 : undefined
          );
        });
      }
      ...
      <Card className="shadow-card">
        <CardHeader>
          ...
          <p className="text-muted-foreground">
            Använd AI för att visualisera förändringar i fastighetsbilder. Ändra färger, möbler, belysning och mer.
          </p>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          <div className="space-y-4">
            ...
            {!user ? (
              <Card className="border-dashed">
                <CardContent className="flex flex-col items-center justify-center py-8 text-center">
                  ...
                  <p className="text-muted-foreground mb-4">
                    Du måste logga in för att använda AI-bildredigeringsverktyget med dina sparade annonser.
                  </p>
                  <Button asChild>
              ...
              <div className="flex items-center justify-center py-8">
                ...
                <p className="text-muted-foreground">Laddar sparade annonser...</p>
              </div>
              ...
              <Card className="border-dashed">
                <CardContent className="flex flex-col items-center justify-center py-8 text-center">
                  ...
                  <p className="text-muted-foreground mb-4">
                    Du har inga sparade annonser än. Spara annonser från sökresultaten för att kunna redigera deras bilder med AI.
                  </p>
          ...
          {/* Edit Options */}
          <Tabs value={editType} onValueChange={(value: any) => setEditType(value)}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-muted-foreground">
                      Hur mycket originalet ska ändras (0.1 = lite, 1.0 = mycket)
                    </p>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    />
                    <p className="text-xs text-muted-foreground">
                      Hur noga AI:n följer instruktionen (1 = kreativ, 20 = exakt)
                    </p>
                  </div>
                </div>
      ...
      {showMasking && selectedImage && (
        <Card className="shadow-card mt-4">
          <CardContent className="pt-6">
            <AIImageMasking
              imageUrl={selectedImage}
              onMaskCreated={(url) => { setMaskDataUrl(url); setShowMasking(false); toast({ title: 'Mask klar', description: 'Masken används nu för precisionsredigering.' }); }}
      ...
      {beforeAfterMode && generatedImages.length > 0 && (
        <Card className="shadow-card">
          <CardHeader>
            ...
            <p className="text-muted-foreground">
              Jämför originalbilden med AI-genererade alternativ. Använd slidern för att jämföra.
            </p>
          </CardHeader>
          <CardContent>
                ...
                {generatedImages.map((img, index) => (
                  <div key={index} className="space-y-4">
                    ...
                    <BeforeAfterSlider
                      ...
                      afterImage={img}
                      onDownload={(type) => handleDownload(
                        type === 'before' ? selectedImage : img,
                        `${type}-${index + 1}.jpg`
                      )}
                      onShare={(type) => handleShare(type === 'before' ? selectedImage : img)}
                    />
                  </div>
                ))}
              </TabsContent>
                      ...
                      <div className="relative group">
                        ...
                        <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity flex gap-2">
                          <Button
                            size="sm"
                            variant="secondary"
                            onClick={() => handleDownload(img, `ai-edited-${index + 1}.jpg`)}
                          >
...
function BeforeAfterSlider({ beforeImage, afterImage, onDownload, onShare }: BeforeAfterSliderProps) {
    ...
    <div className="space-y-4">
      ...
      <Card className="shadow-card">
        <CardHeader>
          ...
          <p className="text-muted-foreground">
            Avancerad AI-bildredigering med OpenAI GPT-Image-1, in-browser AI och professionella verktyg
          </p>
        </CardHeader>
      </Card>
      <div ref={containerRef} className="relative overflow-hidden rounded-lg shadow-lg group">
