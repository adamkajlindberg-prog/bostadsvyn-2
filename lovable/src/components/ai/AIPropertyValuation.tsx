...
import { Calculator, TrendingUp, TrendingDown, BarChart3, MapPin, Home, Calendar, Zap, AlertCircle, CheckCircle } from 'lucide-react';
...
const AIPropertyValuation = () => {
  ...
  const getValuation = async () => {
    if (!user) {
      toast.error('Du måste vara inloggad för att använda värderingsverktyget');
      return;
    }
    setLoading(true);
    try {
      const {
        data,
        error
      } = await supabase.functions.invoke('ai-property-valuation', {
        body: {
          propertyData
        }
      });
      ...
      const result: ValuationResult = {
        estimatedValue: aiValuation.estimatedValue || aiValuation.estimated_value || 0,
        valuationRange: {
          low: aiValuation.valuationRange?.low || aiValuation.valuation_range?.low || aiValuation.estimatedValue * 0.95,
          high: aiValuation.valuationRange?.high || aiValuation.valuation_range?.high || aiValuation.estimatedValue * 1.05
        },
        confidence: aiValuation.confidence || aiValuation.confidence_level || 75,
        marketTrend: (aiValuation.marketTrend?.direction || aiValuation.market_trend || 'stable') as 'up' | 'down' | 'stable',
        trendPercentage: aiValuation.marketTrend?.percentage || aiValuation.trend_percentage || 0,
        comparableProperties: (aiValuation.comparableProperties || aiValuation.comparable_properties || []).map((prop: any) => ({
          address: prop.address || prop.location || 'Okänd adress',
          soldPrice: prop.soldPrice || prop.sold_price || prop.price || 0,
          soldDate: prop.soldDate || prop.sold_date || prop.date || new Date().toISOString(),
          similarity: prop.similarity || 80
        })),
        factors: (aiValuation.factors || aiValuation.value_factors || []).map((factor: any) => ({
          factor: factor.factor || factor.name || factor.title || 'Okänd faktor',
          impact: (factor.impact || 'neutral') as 'positive' | 'negative' | 'neutral',
          description: factor.description || factor.details || ''
        })),
        recommendation: aiValuation.recommendation || aiValuation.recommendations || 'Kontakta en mäklare för mer detaljerad rådgivning.'
      };
  ...
  if (valuationResult) {
    return <div className="space-y-6">
        ...
        <div className="text-center space-y-4">
          ...
          <p className="text-muted-foreground">AI-genererad värdering baserad på marknadsdata</p>
        </div>
          ...
          <CardContent className="p-8">
            <div className="text-center space-y-4">
              <div className="space-y-2">
                <h3 className="text-lg font-semibold text-muted-foreground">Uppskattat marknadsvärde</h3>
                <p className="text-4xl font-bold text-primary">
                  {formatPrice(valuationResult.estimatedValue)}
                </p>
                <p className="text-muted-foreground">
                  Intervall: {formatPrice(valuationResult.valuationRange.low)} - {formatPrice(valuationResult.valuationRange.high)}
                </p>
              </div>
              ...
              <div className="flex items-center justify-center gap-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-medium">Säkerhet: {valuationResult.confidence}%</span>
                </div>
                <div className="flex items-center gap-2">
              ...
              <Progress value={valuationResult.confidence} className="w-full max-w-md mx-auto" />
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-4">
              {valuationResult.comparableProperties.map((property, index) => <div key={index} className="flex items-center justify-between p-4 border rounded-lg">
                  <div className="space-y-1">
                    <p className="font-medium">{property.address}</p>
                    <p className="text-sm text-muted-foreground">
                      Såld: {new Date(property.soldDate).toLocaleDateString('sv-SE')}
                    </p>
                  </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-3">
              {valuationResult.factors.map((factor, index) => <div key={index} className={`p-3 rounded-lg ${getImpactColor(factor.impact)}`}>
  ...
  return <div className="space-y-6">
      ...
      <div className="text-center space-y-4">
        ...
        <p className="text-lg text-muted-foreground">
          Få en professionell värdering av din fastighet med hjälp av AI och marknadsdata
        </p>
      </div>
        ...
        <CardContent className="space-y-4">
          ...
          {step === 1 && <div className="space-y-4">
              <div className="space-y-2">
                ...
                <Input id="address" value={propertyData.address} onChange={e => handleInputChange('address', e.target.value)} placeholder="T.ex. Storgatan 1, Stockholm" />
              </div>

              <div className="space-y-2">
                <Label htmlFor="location">Område/Kommun *</Label>
                <Input id="location" value={propertyData.location} onChange={e => handleInputChange('location', e.target.value)} placeholder="T.ex. Södermalm, Stockholm" />
              </div>
            </div>}
          ...
          {step === 2 && <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Input id="livingArea" type="number" value={propertyData.livingArea || ''} onChange={e => handleInputChange('livingArea', Number(e.target.value))} placeholder="T.ex. 85" />
              </div>

              <div className="space-y-2">
                <Label htmlFor="plotArea">Tomtarea (m²)</Label>
                <Input id="plotArea" type="number" value={propertyData.plotArea || ''} onChange={e => handleInputChange('plotArea', Number(e.target.value))} placeholder="T.ex. 600 (valfritt)" />
              </div>
            </div>}
          ...
          {step === 3 && <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Input id="rooms" type="number" value={propertyData.rooms || ''} onChange={e => handleInputChange('rooms', Number(e.target.value))} placeholder="T.ex. 3" />
                </div>

                <div className="space-y-2">
                  <Label htmlFor="yearBuilt">Byggår *</Label>
                  <Input id="yearBuilt" type="number" value={propertyData.yearBuilt || ''} onChange={e => handleInputChange('yearBuilt', Number(e.target.value))} placeholder="T.ex. 1985" />
                </div>
              </div>

              <div className="space-y-2">
                <Label htmlFor="condition">Skick</Label>
                <Select value={propertyData.condition} onValueChange={value => handleInputChange('condition', value)}>
                  <SelectTrigger>
          ...
          <div className="flex justify-between pt-6">
            ...
            {step < 3 ? <Button onClick={() => setStep(step + 1)} disabled={!validateStep(step)}>
                Nästa
              </Button> : <Button onClick={getValuation} disabled={loading || !validateStep(step)} className="gap-2">
