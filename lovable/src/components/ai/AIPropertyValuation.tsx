...
const AIPropertyValuation = () => {
  ...
  const validateStep = (currentStep: number): boolean => {
    switch (currentStep) {
      case 1:
        return (
          propertyData.address.length > 0 && propertyData.location.length > 0
        );
      case 2:
        return (
          propertyData.propertyType.length > 0 && propertyData.livingArea > 0
        );
      case 3:
      ...
      const result: ValuationResult = {
        ...
        valuationRange: {
          low:
            ...
            aiValuation.estimatedValue * 0.95,
          high:
            aiValuation.valuationRange?.high ||
            aiValuation.valuation_range?.high ||
            aiValuation.estimatedValue * 1.05,
        },
        confidence:
          aiValuation.confidence || aiValuation.confidence_level || 75,
        marketTrend: (aiValuation.marketTrend?.direction ||
          aiValuation.market_trend ||
          "stable") as "up" | "down" | "stable",
        trendPercentage:
          aiValuation.marketTrend?.percentage ||
          aiValuation.trend_percentage ||
          0,
        comparableProperties: (
          aiValuation.comparableProperties ||
          aiValuation.comparable_properties ||
          []
        ).map((prop: any) => ({
        ...
        factors: (aiValuation.factors || aiValuation.value_factors || []).map(
          (factor: any) => ({
            factor:
              factor.factor || factor.name || factor.title || "Okänd faktor",
              ...
              <div className="space-y-2">
                ...
                >
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="renoveringsbehov">
                      Renoveringsbehov
                    </SelectItem>
                  </SelectContent>
                </Select>
        ...
        <Card className="border-primary/20 bg-nordic-ice">
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-semibold text-foreground">Marknadsdata</h3>
                <p className="text-sm text-muted-foreground">
        ...
        <Card className="border-green-200 bg-green-50">
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-semibold text-green-900">AI-analys</h3>
                <p className="text-sm text-green-700">Avancerade algoritmer</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="border-purple-200 bg-purple-50">
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-semibold text-purple-900">Noggrann</h3>
                <p className="text-sm text-purple-700">85-95% säkerhet</p>
              </div>
            </div>
          </CardContent>
        </Card>
