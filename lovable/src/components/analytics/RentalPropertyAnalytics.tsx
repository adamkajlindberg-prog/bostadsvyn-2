...
import { Eye, MessageSquare, TrendingUp, Calendar, Clock, Users } from 'lucide-react';
...
export default function RentalPropertyAnalytics({
  ...
  const loadAnalytics = async () => {
    setLoading(true);
    try {
      await Promise.all([loadViewStatistics(), loadChatStatistics(), loadDailyViewsData()]);
    } catch (error) {
  ...
  const loadViewStatistics = async () => {
    try {
      ...
      const {
        data: allViews
      } = await supabase.from('property_views').select('viewed_at').eq('property_id', propertyId);
      if (allViews) {
        const todayViews = allViews.filter(v => new Date(v.viewed_at) >= startOfToday).length;
        const weekViews = allViews.filter(v => new Date(v.viewed_at) >= startOfWeek).length;
        const monthViews = allViews.filter(v => new Date(v.viewed_at) >= startOfMonth).length;
        setViewStats({
          today: todayViews,
          thisWeek: weekViews,
  ...
  const loadChatStatistics = async () => {
    try {
      ...
      const {
        data: conversations
      } = await supabase.from('conversations').select('id, created_at').eq('property_id', propertyId);
      if (conversations) {
  ...
  const loadDailyViewsData = async () => {
    try {
      ...
      const {
        data: views
      } = await supabase.from('property_views').select('viewed_at').eq('property_id', propertyId).gte('viewed_at', last14Days.toISOString());
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="p-4 border rounded-lg">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm text-muted-foreground">Nya meddelanden</p>
            ...
            <div className="p-4 border rounded-lg">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm text-muted-foreground">Konversationer</p>
                  <p className="text-xl font-bold">{chatStats.totalConversations}</p>
                </div>
              </div>
            </div>
          </div>
