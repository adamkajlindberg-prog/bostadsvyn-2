import { BarChart3, Check, Home, Minus, Plus, Star, X } from "lucide-react";
...
const PropertyComparison = () => {
  ...
  }, [selectedProperties, calculateComparisonMetrics]);
  ...
  const searchProperties = async () => {
    ...
    try {
      const { data, error } = await supabase
        ...
        .eq("status", "FOR_SALE")
        .or(
          `title.ilike.%${searchTerm}%,address_city.ilike.%${searchTerm}%,address_street.ilike.%${searchTerm}%`,
        )
        .limit(10);
  ...
  const calculateComparisonMetrics = () => {
    ...
    selectedProperties.forEach((property) => {
      ...
      const pricePerSqm = property.living_area
      ...
      const priceScore = Math.max(
        0,
        Math.min(100, (80000 - pricePerSqm) / 800),
  ...
  const getBestValue = (
    ...
    const values = Object.values(comparisonMetrics).map(
      (m) => m[metric] as number,
    );
            ...
            <div className="overflow-x-auto">
              <table className="w-full border-collapse">
                <thead>
                  <tr>
                    ...
                    {selectedProperties.map((property) => (
                      <th
                        key={property.id}
                        className="text-center p-2 border-b font-medium min-w-48"
                      >
                        <div className="space-y-2">
                          <div className="flex items-center justify-center gap-2">
                            ...
                            <Button
                              ...
                              variant="ghost"
                              onClick={() =>
                                removePropertyFromComparison(property.id)
                              }
                            >
                              <X className="h-4 w-4" />
                            </Button>
                          ...
                          {comparisonMetrics[property.id] && (
                            <Badge
                              className={`${getScoreColor(comparisonMetrics[property.id].overallScore)} border-0`}
                            >
                              <Star className="h-3 w-3 mr-1" />
                              {comparisonMetrics[property.id].overallScore}{" "}
                              poäng
                            </Badge>
                          )}
                        </div>
                ...
                <tbody>
                    ...
                    {selectedProperties.map((property) => (
                      <td
                        ...
                        className={`text-center p-2 border-b ${
                          isHighlighted(property.id, "pricePerSqm", false)
                  ...
                  <tr>
                    ...
                    {selectedProperties.map((property) => (
                      ...
                      >
                        {comparisonMetrics[property.id] &&
                          formatPricePerSqm(
                            comparisonMetrics[property.id].pricePerSqm,
                          )}
                      </td>
                    ))}
                  </tr>
                    ...
                    {selectedProperties.map((property) => (
                      ...
                      >
                        {property.monthly_fee
                          ? formatPrice(property.monthly_fee)
                          : "Ej angivet"}
                      </td>
                    ))}
                  </tr>
                  ...
                  {/* Market Analysis */}
                  <tr>
                    <td className="p-2 border-b font-medium bg-nordic-ice">
                      Marknadsvärde (AI)
                    </td>
                    {selectedProperties.map((property) => (
                      <td
                        key={property.id}
                        className={`text-center p-2 border-b bg-nordic-ice ${
                          isHighlighted(property.id, "marketValue", true)
                      ...
                      >
                        {comparisonMetrics[property.id] &&
                          formatPrice(
                            comparisonMetrics[property.id].marketValue,
                          )}
                      </td>
                    ))}
                  </tr>
                  ...
                  <tr>
                    ...
                    {selectedProperties.map((property) => (
                      <td
                        key={property.id}
                        className={`text-center p-2 border-b bg-nordic-ice ${
                          isHighlighted(property.id, "walkScore", true)
                            ? "font-semibold text-green-600"
                            : ""
                        }`}
                      >
                  ...
                  <tr>
                    ...
                    {selectedProperties.map((property) => (
                      <td
                        key={property.id}
                        className={`text-center p-2 border-b bg-blue-50 ${
                          isHighlighted(property.id, "schoolRating", true)
                            ? "font-semibold text-green-600"
                            : ""
                        }`}
                      >
                  ...
                  {/* Features Comparison */}
                  <tr>
                    <td className="p-2 border-b font-medium bg-gray-50">
                      Balkong
                    </td>
                    {selectedProperties.map((property) => (
                      <td
                        key={property.id}
                        className="text-center p-2 border-b bg-gray-50"
                      >
                  ...
                  <tr>
                    ...
                    {selectedProperties.map((property) => (
                      <td
                        ...
                        className="text-center p-2 border-b bg-gray-50"
                      >
                        {hasFeature(property, "garage") ||
                        hasFeature(property, "parkering") ? (
                          <Check className="h-5 w-5 text-green-500 mx-auto" />
                        ) : (
                          <Minus className="h-5 w-5 text-gray-400 mx-auto" />
                        )}
                  ...
                  <>
                    <p>
                      ...
                      {selectedProperties.find((p) =>
                        isHighlighted(p.id, "pricePerSqm", false),
                      )?.title || "N/A"}
                    </p>
                    <p>
                      • <strong>Högsta tillväxtpotential:</strong>{" "}
                      {selectedProperties.find((p) =>
                        isHighlighted(p.id, "potentialAppreciation", true),
                      )?.title || "N/A"}
                    </p>
                    <p>
                      • <strong>Bästa läge:</strong>{" "}
                      {selectedProperties.find((p) =>
                        isHighlighted(p.id, "walkScore", true),
                      )?.title || "N/A"}
                    </p>
                  </>
                )}
