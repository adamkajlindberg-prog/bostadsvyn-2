...
export default function MarketShareStats() {
  ...
  const [_officeData, setOfficeData] = useState<any>(null);
  ...
  const [showNewConstruction, setShowNewConstruction] = useState<
    "hide" | "show" | "only"
  >("hide");
  ...
  const [_tenureType, _setTenureType] = useState<string[]>(["all"]);
  const [hasBalcony, setHasBalcony] = useState(false);
  const [hasParking, setHasParking] = useState(false);
  const [hasElevator, setHasElevator] = useState(false);

  // Stats
  const [_brokerStats, setBrokerStats] = useState<MarketStats>({
    totalProperties: 0,
    soldProperties: 0,
    comingSoon: 0,
    marketShare: 0,
  ...
  const [_officeStats, setOfficeStats] = useState<MarketStats>({
    totalProperties: 0,
    soldProperties: 0,
    comingSoon: 0,
    marketShare: 0,
  ...
  const [_propertyTypeStats, setPropertyTypeStats] = useState<
    PropertyTypeStats[]
  >([]);
  ...
  const loadStatistics = async () => {
    ...
    try {
      ...
      const brokerSold = brokerProperties.filter(
        (p) => p.status === "SOLD",
      ).length;
      const brokerComingSoon = brokerProperties.filter(
        (p) => p.status === "COMING_SOON",
      ).length;
      const brokerAvgPrice =
        brokerProperties.length > 0
          ? brokerProperties.reduce((sum, p) => sum + (p.price || 0), 0) /
            brokerProperties.length
          : 0;
      const brokerTotalArea = brokerProperties.reduce(
        (sum, p) => sum + (p.living_area || 0),
        0,
      );

      // Calculate office stats
      const officeProperties = officeResult.data || [];
      const officeSold = officeProperties.filter(
        (p) => p.status === "SOLD",
      ).length;
      const officeComingSoon = officeProperties.filter(
        (p) => p.status === "COMING_SOON",
      ).length;
      const officeAvgPrice =
        officeProperties.length > 0
          ? officeProperties.reduce((sum, p) => sum + (p.price || 0), 0) /
            officeProperties.length
          : 0;
      const officeTotalArea = officeProperties.reduce(
        (sum, p) => sum + (p.living_area || 0),
        0,
      );
  ...
  const loadPropertyTypeStatistics = async () => {
    ...
    try {
      ...
      const typeStatsPromises = propertyTypesArray.map(async (type) => {
        ...
        const brokerProperties = brokerResult.data || [];
        const totalMarket = marketResult.count || 1;
        const soldCount = brokerProperties.filter(
          (p) => p.status === "SOLD",
        ).length;
        const avgPrice =
          brokerProperties.length > 0
  ...
  const loadAllBrokersRanking = async () => {
    try {
      ...
      setAllBrokersRanking(
        rankings.sort((a, b) => b.totalProperties - a.totalProperties),
      );
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="flex">
        {/* Sidebar Filter */}
        <aside className="w-80 border-r bg-gradient-to-b from-card to-muted/20 p-6 h-screen overflow-y-auto sticky top-0">
          <Button
            variant="ghost"
            onClick={() => navigate("/maklare-portal")}
          ...
          <div className="mb-6">
            <h2 className="text-xl font-bold mb-2 bg-gradient-to-r from-primary to-primary/60 bg-clip-text text-transparent">
              Analysfilter
            </h2>
            <p className="text-xs text-muted-foreground">
              Anpassa din marknadsanalys
            </p>
          </div>

          <Accordion
            type="multiple"
            defaultValue={["type", "area", "price"]}
            className="space-y-2"
          >
            {/* Bostadstyp */}
            <AccordionItem
            ...
            >
              <AccordionTrigger className="hover:no-underline">
                <div className="flex items-center gap-2">
                  <Building2 className="h-4 w-4" />
                  <span className="font-medium">Avancerat</span>
                </div>
              </AccordionTrigger>
              <AccordionContent className="pt-4 pb-2 space-y-4">
                <div>
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    <Button
                      size="sm"
                      variant={
                        showNewConstruction === "hide" ? "default" : "outline"
                      }
                      onClick={() => setShowNewConstruction("hide")}
                      className="flex-1 h-9"
                    >
                      Dölj
                    </Button>
                    <Button
                      size="sm"
                      variant={
                        showNewConstruction === "show" ? "default" : "outline"
                      }
                      onClick={() => setShowNewConstruction("show")}
                      className="flex-1 h-9"
                    >
                      Visa
                    </Button>
                    <Button
                      size="sm"
                      variant={
                        showNewConstruction === "only" ? "default" : "outline"
                      }
                      onClick={() => setShowNewConstruction("only")}
                      className="flex-1 h-9"
                    >
                      Endast
                    </Button>
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    <Button
                      size="sm"
                      variant={showCondos === "hide" ? "default" : "outline"}
                      onClick={() => setShowCondos("hide")}
                      className="flex-1 h-9"
                    >
                      Dölj
                    </Button>
                    <Button
                      size="sm"
                      variant={showCondos === "show" ? "default" : "outline"}
                      onClick={() => setShowCondos("show")}
                      className="flex-1 h-9"
                    >
                      Visa
                    </Button>
        ...
        <main className="flex-1 p-8">
          <div className="max-w-7xl mx-auto">
            <div className="mb-8">
              <h1 className="text-4xl font-bold mb-2">Andelsstatistik</h1>
              ...
              <CardContent className="pt-6">
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="flex gap-2">
                      {(
                        ["utbud", "nya", "borttagna", "salda"] as StatusFilter[]
                      ).map((status) => (
                        <Button
                          key={status}
                          size="sm"
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Baserat på antal aktiva objekt under vald period
                </CardDescription>
              </CardHeader>
              <CardContent>
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead>
                        {viewType === "broker" ? "Mäklare" : "Kontor"}
                      </TableHead>
                  ...
                  <TableBody>
                    {viewType === "broker" &&
                      allBrokersRanking.slice(0, 20).map((broker, index) => (
                        <TableRow
                          key={broker.broker_id}
                          className={
                            broker.broker_id === user?.id ? "bg-primary/10" : ""
                          }
                        >
                    ...
                    {viewType === "office" &&
                      allOfficesRanking.slice(0, 20).map((office, index) => (
                        <TableRow
                          key={office.office_id}
                          className={
                            office.office_id === brokerData?.office_id
                              ? "bg-primary/10"
                              : ""
                          }
                        >
                          ...
                          <TableCell>
                            <div>
                              ...
                              <p className="text-sm text-muted-foreground">
                                {office.totalBrokers} mäklare
                              </p>
                            </div>
                          </TableCell>
                          <TableCell className="text-right font-medium">
                            {office.marketShare.toFixed(1)}%
                          </TableCell>
                          <TableCell className="text-right">
                          ...
                          <TableCell className="text-right">
                            {office.totalProperties}
                          </TableCell>
                          <TableCell className="text-right">-</TableCell>
                          <TableCell className="text-right">-</TableCell>
                          <TableCell className="text-right">
                            {office.soldProperties}
                          </TableCell>
                        </TableRow>
                      ))}
                  </TableBody>
