...
const NyproduktionDetail = () => {
  ...
  React.useEffect(() => {
    const fetchProjectData = async () => {
      ...
      try {
        ...
        // @ts-expect-error - Suppress type inference depth error
  ...
  const availableUnitsCount = units.filter(
    (u) => u.status === "FOR_SALE" || u.status === "COMING_SOON",
  ).length;
  const minPrice =
    units.length > 0 ? Math.min(...units.map((u) => u.price)) : 0;
  const soldPercentage =
    project.nyproduktion_total_units > 0
      ? Math.round(
          (1 - availableUnitsCount / project.nyproduktion_total_units) * 100,
        )
      : 0;

  const _projectForDisplay = {
    ...
    fullDescription: (project.description ||
      "Information om projektet kommer snart.") as string,
    priceFrom:
      units.length > 0
        ? `${(minPrice / 1000000).toFixed(1)}M kr`
        : "Pris kommer",
    image:
      (project.images?.[0] as string) ||
      "/lovable-uploads/6460350a-4407-412d-802c-ca99c2bfd9e3.png",
    ...
    completionDate: project.year_built
      ? `Q${Math.ceil((new Date().getMonth() + 1) / 3)} ${project.year_built}`
      : "Ej angiven",
          ...
          <div className="flex items-start justify-between mb-6">
            ...
            <div className="text-right">
              <div className="text-3xl font-bold text-premium mb-2">
                Från {project.priceFrom}
              </div>
              <div className="flex items-center gap-2 justify-end">
                <Star className="h-5 w-5 text-accent fill-current" />
                <span className="text-sm">
                  {project.energyClass} energiklass
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {project.units.map((unit) => (
              <Card
                key={unit.id}
                className={!unit.available ? "opacity-50" : ""}
              >
                <CardHeader>
                  ...
                  <div className="space-y-3 mb-4">
                    ...
                    <div className="flex items-center justify-between">
                      <span className="text-2xl font-bold">
                        {(unit.price / 1000000).toFixed(1)}M kr
                      </span>
                      <span className="text-sm text-muted-foreground">
                        {Math.round(unit.price / unit.area)} kr/m²
                      </span>
                    </div>
                  </div>
                  {unit.available && (
          ...
          <CardContent className="p-8 text-center">
            ...
            <div className="flex gap-4 justify-center">
              <Button
                size="lg"
                variant="outline"
                className="border-premium-foreground text-premium-foreground hover:bg-premium-foreground hover:text-premium"
              >
