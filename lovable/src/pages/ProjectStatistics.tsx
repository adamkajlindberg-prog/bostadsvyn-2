...
const ProjectStatistics = () => {
  ...
  const loadProjectData = async () => {
    try {
      ...
      const unitsAvailable = (unitsData || []).filter(
        (u) => u.status === "FOR_SALE",
      ).length;
      const unitsReserved = (unitsData || []).filter(
        (u) => u.status === "RESERVED",
      ).length;
      const unitsSold = (unitsData || []).filter(
        (u) => u.status === "SOLD",
      ).length;
          ...
          <TabsContent value="statistics" className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold">
                    {stats.unitsSold + stats.unitsReserved}
                  </div>
                  <p className="text-xs text-muted-foreground">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  <div>
                    ...
                    <p className="font-medium">
                      {project.nyproduktion_total_units} st
                    </p>
                  </div>
                  {project.year_built && (
                    <div>
                  ...
                  <div>
                    ...
                    <Badge
                      variant={
                        project.status === "FOR_SALE" ? "default" : "secondary"
                      }
                    >
                      {project.status === "FOR_SALE"
                        ? "Till salu"
                        : project.status}
                    </Badge>
                  </div>
