...
serve(async (req) => {
  ...
  try {
    ...
    const computedStrength = Math.min(
      0.35,
      Math.max(0.15, typeof reqStrength === "number" ? reqStrength : 0.25),
    );
    const computedGuidance = Math.min(
      12,
      Math.max(3, typeof reqGuidance === "number" ? reqGuidance : 7.5),
    );
    const steps = 18; // slightly higher for quality while keeping speed

    const _preservationPrefix =
      "PRESERVE ORIGINAL ROOM: Keep layout, architecture, furniture placement, camera angle and lighting. Only change requested elements.";
    const builtInNegatives =
      "Do not change room layout, do not change furniture style or placement, do not change camera angle, do not change color palette unless explicitly requested, avoid surreal/stylized looks, avoid extra objects, avoid duplicates.";
    const finalNegativePrompt = negativePrompt
      ? `${builtInNegatives} ${negativePrompt}`
      : builtInNegatives;
    ...
    console.log("Environment check:", {
      ...
      allEnvKeys: Object.keys(Deno.env.toObject()).filter(
        (key) =>
    ...
    if (hfToken) {
      try {
        ...
        if (!imageResponse.ok) {
          throw new Error(
            `Failed to fetch original image: ${imageResponse.status} - ${absoluteImageUrl}`,
          );
        }
        const imageBlob = await imageResponse.blob();
        console.log(
          "Successfully fetched original image, size:",
          imageBlob.size,
        );
...
Professional interior photography style, natural lighting, high detail.`;
        ...
        if (maskUrl) {
          ...
          if (!maskResponse.ok) {
            throw new Error(
              `Failed to fetch mask image: ${maskResponse.status}`,
            );
          }
          ...
          response = await fetch(
            "https://api-inference.huggingface.co/models/diffusers/controlnet-inpainting-sd-xl-1.0",
            {
              method: "POST",
        ...
        if (response.ok) {
          ...
          console.log(
            "FLUX image-to-image editing completed successfully with preservation",
          );
              ...
              images: [
                {
                  ...
                  model_used: maskUrl
                    ? "ControlNet-Inpainting-SDXL"
                    : "FLUX.1-schnell-image2image",
                  variant: 1,
                },
              ],
              metadata: {
                ...
                editing_mode: maskUrl
                  ? "precise-inpainting"
                  : "image-to-image-preservation",
        ...
        const hfErrorMessage =
          hfError instanceof Error ? hfError.message : "Unknown HF error";
              ...
              images: [
                {
                  ...
                  revised_prompt:
                    openAIData.data[0].revised_prompt || refinedPrompt,
                  model_used: "dall-e-3",
                  variant: 1,
                },
              ],
    ...
    return new Response(
      JSON.stringify({
        ...
        debug: {
          ...
          environmentKeys: Object.keys(Deno.env.toObject()).filter(
            (key) =>
              key.includes("HUG") ||
              key.includes("OPENAI") ||
              key.includes("API"),
