...
serve(async (req) => {
  ...
  try {
    ...
    const { data: searchHistory, error: historyError } = await supabase
      ...
      .select('search_query, search_filters, property_type, price_range, location')
      .eq('user_id', user_id)
    ...
    if (historyError) {
      ...
      return new Response(JSON.stringify({ error: 'Failed to fetch search history' }), {
        status: 500,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    if (!searchHistory || searchHistory.length === 0) {
      return new Response(JSON.stringify({ 
        recommendations: [],
        message: 'Inga sökningar ännu - börja söka för att få AI-rekommendationer!' 
      }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
  ...
  "confidence": 0.8
    ...
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        model: 'gpt-4o-mini',
        messages: [
          { role: 'system', content: 'Du är en AI-assistent som analyserar fastighetsönskemål och skapar databassökningar. Svara endast med valid JSON.' },
    ...
    if (aiAnalysis.price_range) {
      query = query.gte('price', aiAnalysis.price_range.min).lte('price', aiAnalysis.price_range.max);
    }

    // Location filter
    if (aiAnalysis.locations && aiAnalysis.locations.length > 0) {
      const locationFilter = aiAnalysis.locations.map((loc: string) => `address_city.ilike.%${loc}%`).join(',');
