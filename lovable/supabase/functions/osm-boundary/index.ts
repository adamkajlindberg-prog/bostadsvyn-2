...
function geometryArea(geometry: any): number {
  ...
  if (geometry.type === 'Polygon') {
    ...
    return rings.reduce((sum: number, ring: number[][]) => sum + ringArea(ring), 0);
  }
  if (geometry.type === 'MultiPolygon') {
    return geometry.coordinates.reduce((sum: number, poly: number[][][]) => sum + poly.reduce((s, ring) => s + ringArea(ring), 0), 0);
  }
  return 0;
}
...
async function resolveMunicipalityFeature(query: string): Promise<any | null> {
  ...
  let best: { item: any; geometry: any; area: number; score: number } | null = null;
  ...
  for (const c of candidates) {
    ...
    const t = c.tags || {};
    const names = [t['official_name'], t['official_name:sv'], t['name'], t['name:sv'], c.display_name, c.name]
      .filter(Boolean)
  ...
  const display = (() => {
    ...
    const names = [t['official_name'], t['official_name:sv'], t['name'], t['name:sv']].filter(Boolean) as string[];
...
serve(async (req) => {
  ...
  try {
    ...
    const isList = url.searchParams.get('list') === 'true' || listInBody === true;
    ...
    for (const searchQuery of searchQueries) {
      ...
      if (type === 'municipality' || searchQuery.toLowerCase().includes('kommun')) {
      ...
      if (allItems.length > 0 && allItems.some((item: any) => 
        item.class === 'boundary' && item.type === 'administrative' && 
        (item.extratags?.admin_level === '8' || item.extratags?.admin_level === 8)
      )) {
        break;
      }
    }
    ...
    const uniqueItems = allItems.filter((item, index, arr) => 
      arr.findIndex(i => i.osm_id === item.osm_id && i.osm_type === item.osm_type) === index
    );

    let items = uniqueItems;

    // Fallback/augment with Overpass for Swedish municipalities when needed
    if (isList && (type === 'municipality' || q.toLowerCase().includes('kommun'))) {
      try {
        const extra = await overpassMunicipalitySearch(q, 10);
        if (extra?.length) {
          items = [...items, ...extra].filter((item, index, arr) =>
            arr.findIndex(i => i.osm_id === item.osm_id && i.osm_type === item.osm_type) === index
          );
        }
      } catch (_) {}
    ...
    if (!items || items.length === 0) {
      ...
      return new Response(JSON.stringify({ error: "No results", query: q }), { status: 404, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    ...
    if (isList) {
      ...
      const toKey = (s: any) => {
        if (s.type === 'municipality') {
          const base = String(s.name || '').replace(/\s*kommun\s*$/i, '').trim().toLowerCase();
          return `muni|${base}`;
        }
        if (s.type === 'district') {
      ...
      for (const i of items) {
        const adminRaw = i?.extratags?.admin_level ?? i?.tags?.admin_level ?? i?.admin_level ?? i?.address?.admin_level;
        const adminLevel = typeof adminRaw === 'string' ? adminRaw : String(adminRaw ?? '');
        const addresstype = (i?.addresstype || '').toLowerCase();
        const cls = i?.class;
        const typ = i?.type;
        const nameStr = i?.name || i?.address?.name || '';
        const disp = i?.display_name || '';
        const endsKommun = /\bkommun\b/i.test(nameStr) || /\bkommun\b/i.test(disp);
        ...
        const muni = i?.address?.municipality || i?.address?.city || i?.address?.town || i?.address?.county || undefined;
        
        // Check for actual Swedish municipality (admin_level 8 is definitive)
        const isAdminLevel8 = (cls === 'boundary' && typ === 'administrative' && adminLevel === '8');
        const isExplicitMunicipality = (addresstype === 'municipality' || typ === 'municipality');
        ...
        if (isAdminLevel8 || isExplicitMunicipality) {
          ...
          const hasConflictingMuni = muni && !muni.toLowerCase().includes(nameStr.replace(/\s*kommun\s*$/i, '').trim().toLowerCase());
          if (!hasConflictingMuni) {
            normType = 'municipality';
          }
        } else if ((cls === 'boundary' && typ === 'administrative' && (adminLevel == '9' || adminLevel == '10'))
          || (cls === 'place' && ['suburb','neighbourhood','borough','quarter','city_district'].includes(String(typ)))) {
          normType = 'district';
        } else {
          normType = 'address';
        }
        ...
        if (normType === 'municipality') {
          ...
          const pretty = (base || '').toString().replace(/\s*kommun\s*$/i, '').trim() + ' kommun';
          name = pretty;
          fullName = pretty;
        } else if (normType === 'district') {
          const district = i?.name || i?.address?.suburb || i?.address?.neighbourhood || i?.address?.quarter || i?.address?.borough || name;
          name = district;
      ...
      out.sort((a: { type: keyof typeof order }, b: { type: keyof typeof order }) => order[a.type] - order[b.type]);
      ...
      try {
        ...
        console.log('[osm-boundary] list', { q, type, total_in: items.length, out_total: out.length, muniCount, distCount, addrCount });
      } catch {}

      return new Response(JSON.stringify({ suggestions: out.slice(0, 15) }), { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    ...
    if (type === 'municipality' || q.toLowerCase().includes('kommun')) {
      try {
        ...
        if (resolved?.geometry) {
          return new Response(JSON.stringify({ feature: resolved }), { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } });
        }
      } catch (_) {}
    }
    ...
    const pick = (arr: any[], typeHint?: string) => {
      ...
      if (typeHint === 'municipality' || q.toLowerCase().includes('kommun')) {
        const m = arr.find(i => i.class === 'boundary' && i.type === 'administrative' && 
          (i.extratags?.admin_level === '8' || i.extratags?.admin_level === 8));
        if (m) return m;
        // Fallback: any administrative boundary for municipalities
        const admin = arr.find(i => i.class === 'boundary' && i.type === 'administrative');
        if (admin) return admin;
      }
      // District: admin boundary level 9-10 or place=suburb/neighbourhood
      if (typeHint === 'district') {
        const d = arr.find(i => (i.class === 'boundary' && i.type === 'administrative' && 
          (i.extratags?.admin_level === '10' || i.extratags?.admin_level === '9')) || 
          (i.class === 'place' && (i.type === 'suburb' || i.type === 'neighbourhood')));
        if (d) return d;
      }
      // Street/address: highway or building with addr
      if (typeHint === 'street' || typeHint === 'address') {
        const s = arr.find(i => (i.class === 'highway') || (i.class === 'building') || (i.address?.road));
        if (s) return s;
      }
      // General priority: administrative boundaries first, then any with polygon
      const admin = arr.find(i => i.class === 'boundary' && i.type === 'administrative' && i.geojson && 
        (i.geojson.type === 'Polygon' || i.geojson.type === 'MultiPolygon'));
      if (admin) return admin;
      // Any with polygon
      const poly = arr.find(i => i.geojson && (i.geojson.type === 'Polygon' || i.geojson.type === 'MultiPolygon'));
      if (poly) return poly;
      // Fall back to first
      return arr[0];
    };
    ...
    if ((type === 'municipality' || q.toLowerCase().includes('kommun'))) {
      const hasPolygon = finalChoice?.geojson && (finalChoice.geojson.type === 'Polygon' || finalChoice.geojson.type === 'MultiPolygon' || finalChoice.geojson.type === 'Feature' || finalChoice.geojson.type === 'FeatureCollection');
      const isMunicipality = finalChoice?.class === 'boundary' && finalChoice?.type === 'administrative' && (finalChoice?.extratags?.admin_level == '8' || finalChoice?.extratags?.admin_level == 8);
    ...
    if (type === 'district') {
      const hasPolygon = finalChoice?.geojson && (finalChoice.geojson.type === 'Polygon' || finalChoice.geojson.type === 'MultiPolygon' || finalChoice.geojson.type === 'Feature' || finalChoice.geojson.type === 'FeatureCollection');
      const isDistrict = finalChoice?.class === 'boundary' && finalChoice?.type === 'administrative' && (finalChoice?.extratags?.admin_level == '9' || finalChoice?.extratags?.admin_level == 9 || finalChoice?.extratags?.admin_level == '10' || finalChoice?.extratags?.admin_level == 10);
    ...
    const hasPolyGeom = ensured?.geojson && ((ensured.geojson.type === 'Polygon') || (ensured.geojson.type === 'MultiPolygon') || (ensured.geojson.type === 'Feature') || (ensured.geojson.type === 'FeatureCollection'));
    if (!hasPolyGeom && ensured?.osm_id) {
      const osmTypeRaw = ensured.osm_type;
      const letter: 'R' | 'W' | 'N' = (osmTypeRaw === 'relation' || osmTypeRaw === 'R') ? 'R' : (osmTypeRaw === 'way' || osmTypeRaw === 'W') ? 'W' : 'N';
      try {
    ...
    if ((type === 'municipality' || q.toLowerCase().includes('kommun'))) {
      ...
      const stillNoPolygon = !g || (g.type === 'Point') || (g.type === 'LineString') || (g.type === 'MultiLineString');
      if (stillNoPolygon) {
        try {
          const over = await overpassMunicipalitySearch(q, 1);
          const first = over?.[0];
          if (first?.osm_id) {
            const fr = await fetchOSMFranceGeoJSONForRelation(Number(first.osm_id));
            if (fr) {
              ensured = { ...ensured, geojson: fr, class: 'boundary', type: 'administrative', extratags: { admin_level: '8' }, display_name: ensured?.display_name || q };
            }
          }
        } catch (_) {}
      }
    ...
    if (g) {
      ...
      else if (g.type === 'FeatureCollection') geometry = g.features?.[0]?.geometry || null;
      else geometry = g;
    }
    ...
    if (!geometry && finalChoice?.boundingbox) {
      geometry = {
        type: 'Polygon',
        coordinates: [[
          [Number(finalChoice.boundingbox[2]), Number(finalChoice.boundingbox[0])],
          [Number(finalChoice.boundingbox[3]), Number(finalChoice.boundingbox[0])],
          [Number(finalChoice.boundingbox[3]), Number(finalChoice.boundingbox[1])],
          [Number(finalChoice.boundingbox[2]), Number(finalChoice.boundingbox[1])],
          [Number(finalChoice.boundingbox[2]), Number(finalChoice.boundingbox[0])],
        ]],
      };
    }
    ...
    return new Response(JSON.stringify({ feature }), { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } });
  } catch (e) {
