...
serve(async (req) => {
  ...
  try {
    ...
    console.log("Verifying content:", { title, contentType, imageCount: images.length });
...
Är detta innehåll lämpligt för publicering?`
    ...
    try {
      ...
      verification = {
        approved: aiResponse.toLowerCase().includes('approved') || aiResponse.toLowerCase().includes('godkänd'),
        reason: aiResponse
      };
    }
