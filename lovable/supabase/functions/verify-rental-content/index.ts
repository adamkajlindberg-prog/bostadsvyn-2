...
serve(async (req) => {
  ...
  try {
    ...
    const textVerificationResponse = await fetch(
...
{"approved": true/false, "reason": "detaljerad förklaring på svenska om varför innehållet godkänns eller avvisas", "violations": ["lista över specifika överträdelser om några"]}`,
      ...
      if (textVerificationResponse.status === 429) {
        return new Response(
          JSON.stringify({
            approved: false,
            reason:
              "AI-verifieringstjänsten är överbelastad. Försök igen om en stund.",
          }),
          {
            status: 429,
      ...
      if (textVerificationResponse.status === 402) {
        return new Response(
          JSON.stringify({
            approved: false,
            reason:
              "AI-verifieringstjänsten är tillfälligt otillgänglig. Kontakta support.",
          }),
          {
            status: 402,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    try {
      ...
      textResult = content.includes("{")
        ? JSON.parse(content)
        : { approved: false, reason: content };
    } catch (parseError) {
      console.error("Failed to parse AI response:", parseError);
      textResult = {
        approved: false,
        reason: "Kunde inte verifiera innehållet. Försök igen.",
      };
    }
    ...
    if (hasAIEditedImages && !markedAsPhotomontage) {
      return new Response(
        JSON.stringify({
          approved: false,
          reason:
            "AI-redigerade bilder måste märkas som fotomontage enligt FMI:s riktlinjer. Markera detta i din annons.",
    ...
    if (imageUrls.length > 0) {
      ...
      for (let i = 0; i < imagesToCheck.length; i++) {
        ...
        try {
          const imageVerificationResponse = await fetch(
            "https://ai.gateway.lovable.dev/v1/chat/completions",
            {
              method: "POST",
              headers: {
...
Svara ENDAST med JSON: {"approved": true/false, "reason": "förklaring på svenska"}`,
          ...
          if (imageVerificationResponse.ok) {
            const imageVerificationData =
              await imageVerificationResponse.json();
            const imageContent =
              imageVerificationData.choices[0].message.content;

            let imageResult;
            try {
              imageResult = imageContent.includes("{")
                ? JSON.parse(imageContent)
                : { approved: true };
            } catch {
    ...
    return new Response(
      JSON.stringify({
        approved: true,
        reason:
          "Innehållet har godkänts. Annonsen följer svensk lag och FMI:s riktlinjer.",
        violations: [],
      }),
      {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
