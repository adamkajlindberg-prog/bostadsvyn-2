...
-- Add moderation status to properties (drop existing constraint first)
DO $$ BEGIN
  ALTER TABLE public.properties DROP CONSTRAINT IF EXISTS check_moderation_status;
...
-- Add ads moderation
DO $$ BEGIN
  ALTER TABLE public.ads DROP CONSTRAINT IF EXISTS check_ads_moderation_status;
EXCEPTION WHEN undefined_object THEN NULL;
END $$;
