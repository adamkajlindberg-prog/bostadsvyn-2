...
const apiMap: Record<string, string> = {
    ...
    "cadastral-parcel": "https://api.lantmateriet.se/fastighetsomrade/atom/v1/inspire/cp",
    "physical-water": "https://api.lantmateriet.se/hydrografi/atom/v1.1/inspire/hyp",
    "hydro-network": "https://api.lantmateriet.se/hydrografi/atom/v1.1/inspire/hyn",
...
const extractEntries = (xml: string) => {
    ...
    return entryList.map(entry => {
        ...
        const polygon = polygonRaw
            ? polygonRaw.trim().split(/\s+/).reduce<{ longitude: number, latitude: number }[]>((acc, val, idx, arr) => {
                if (idx % 2 === 0 && arr[idx + 1]) {
                    const longitude = parseFloat(val)
                    const latitude = parseFloat(arr[idx + 1])
...
const populateLantmaterietData = async () => {
    try {    
        if (!categoryArg) {
            console.error(`Error: You must provide category (--category="") in the script.`)
            process.exit(1)
        }        
        ...
        console.log(`SUCCESS! Lantmäteriet ${categoryArg} data populated successfully.`)
    } catch (error) {
        console.error(`Failed to populate Lantmäteriet ${categoryArg} data:`, error)
    }
}
