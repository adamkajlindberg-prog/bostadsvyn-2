...
const getAllScb = async () => {
    const response = await fetch("https://api.scb.se/OV0104/v2beta/api/v2/tables")
...
const populateScbData = async () => {
    ...
    for (let i = 0; i < scbTables.length; i++) {
        ...
        console.log(`Upserting data [${table.id}] (${i + 1}/${scbTables.length})...`)

        const tableData = await fetch(`https://api.scb.se/OV0104/v2beta/api/v2/tables/${table.id}/data?outputFormat=px`)
