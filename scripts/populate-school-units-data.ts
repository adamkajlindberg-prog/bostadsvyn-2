...
const getExistingData = async () => {
    const data = await db.select({ schoolUnitCode: schoolUnits.schoolUnitCode }).from(schoolUnits)
...
const getAllSchoolUnits = async () => {
    const response = await fetch("https://api.skolverket.se/skolenhetsregistret/v2/school-units")
...
const retrieveData = async (schoolUnitCode: string) => {
    const response = await fetch(`https://api.skolverket.se/skolenhetsregistret/v2/school-units/${schoolUnitCode}`)
    if(!response.ok) throw new Error(`HTTP error! status: ${response.status}`)
...
const populateSchoolUnitsData = async () => {
    try {
        ...
        const schoolUnitCodes = schoolUnits.data.attributes.map((unit: T_School_Unit) => unit.schoolUnitCode) // extract school unit codes from API
        const freshCodes = schoolUnitCodes.filter((code: string) => !existingData.includes(code)) // filter out existing codes
        ...
        for (const batchCodes of batches) {
            ...
            console.log(`Inserting batch ${batchNumber + 1}/${totalBatches} (${batchCodes.length} school units)...`)
        ...
        console.error("Fetch failed (Skolverket API - School Units). Error:", error)
    }
}
