...
const populateSwestrCompoundedAverageData = async (dateFrom: string, dateTo: string) => {
    if (!dateFrom || !dateTo) {
        console.error(`Error: You must provide date range (--dateFrom="YYYY-MM-DD" --dateTo="YYYY-MM-DD") in the script.`)
        process.exit(1)
    }

    console.log("Retrieving data from Riksbank API (SWESTR - Compounded Average)...")
    console.log(`Using date range: ${dateFrom} to ${dateTo}`)
    
    const response = await fetch(`https://api.riksbank.se/swestr/v1/avg/SWESTRAVG1W?fromDate=${dateFrom}&toDate=${dateTo}`)
    ...
    console.log("SUCCESS! All SWESTR compounded average data populated successfully.")
}
