...
const populateSwestrInterestRateData = async (dateFrom: string, dateTo: string) => {
    if (!dateFrom || !dateTo) {
    ...
    const response = await fetch(`https://api.riksbank.se/swestr/v1/SWESTR?fromDate=${dateFrom}&toDate=${dateTo}`)
